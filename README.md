...
> --exit-code`, on Windows and Linux.)
...
— no number in this README is typed by hand, and CI fails if regeneration
produces anything different.
...
| Crossover: below a small corpus size, stuffing is cheaper | **mini corpus (8 notes): selective 431 vs stuff 340 proxy tokens (ratio 1.27)** | same harness on an 8-note wiki whose tasks touch most of the corpus | selective recall pays the index every session; on a small, hot corpus that overhead is pure loss — the design only wins when the wiki outgrows its working set |
| One-line hooks recover the labeled relevant notes | **precision 0.95 / recall 0.95 (20/21 recalled correct, 20/21 relevant found, 1 shown miss(es))** | every task carries author-written ground-truth labels; the runner logs actual recalls | an upper bound by construction — the same author wrote tasks, hooks, and labels; the deliberately lazy hook produces the shown miss |
...
| Memory stays smaller than what it remembers | **final wiki 4387 bytes vs 12792 bytes of session transcripts** | byte sizes of the generated wiki vs the transcripts it was distilled from | growth depends on the extend/prune discipline the protocol enforces; a corpus without corrections would grow differently |
