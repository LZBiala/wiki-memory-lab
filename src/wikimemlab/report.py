...
def _dir_bytes(path: Path, suffix: str = ".md") -> int:
    return sum(len(p.read_bytes()) for p in sorted(path.glob(f"*{suffix}")))
...
def render_claims(rows: list[Row], wiki_dir: Path, runs_dir: Path) -> str:
    ...
    wiki_bytes = _dir_bytes(wiki_dir)
    transcript_bytes = _dir_bytes(runs_dir)
    ...
    lines = [
        ...
        (
            ...
            f"| **{sel_total} vs {stuff_total} proxy tokens over 8 sessions "
        ...
        (
            ...
            f"| **mini corpus (8 notes): selective {mini_measure_sel} vs stuff "
            f"{mini_measure_stuff} proxy tokens (ratio {mini_ratio:.2f})** | same "
            "harness on an 8-note wiki whose tasks touch most of the corpus | "
        ...
        (
            ...
            f"{misses} shown miss(es))** | every task carries author-written "
            "ground-truth labels; the runner logs actual recalls | an upper bound by "
        ...
        (
            ...
            f"(~{per_note:.1f}/note, {overhead:.2f} of full-corpus cost at session 8)** "
        ...
        (
            ...
            f"| **final wiki {wiki_bytes} bytes vs {transcript_bytes} bytes of session "
            "transcripts** | byte sizes of the generated wiki vs the transcripts it "
            "was distilled from | growth depends on the extend/prune discipline the "
            "protocol enforces; a corpus without corrections would grow differently |"
        ),
    ]
    return "\n".join(lines)
